...
#	return ;
			...
			#	$title_temp =~ s/&#(\d+);//gi;
				...
				my ($title,$artist ) = split(/ Lyrics /ig, $title_temp);
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#<!-------PASTE SONG LYRICS AND ALL INFO BETWEEN THESE TWO LINES--->\s*(.*?)\s*</div></pre></h1>#igs);
		...
		#		$content = encode($enc, $songtext);
				...
				#	if ($entry =~ /linenums/ig){
				#		(undef,$entry) = split(">",$entry);
				#	}
					next if ($entry =~ /(\bad\b|\:)/ig);
					$c .= "$entry<br />\n";
