...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /([a-z0-9])/ig && $SourceFilename !~ m/index/ig && $size > 2000){	
...
#				return;
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#<div id="showtext">\s*(.*?)\s*<div id="socialshareprivacy"></div>#igs);
