...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /([a-z0-9]).html\?p=/ig && -e $SourceFilename ){	
...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				my @split = split("\n", $string);				
				my ($content) = ($string =~ m#<div style='padding:0px 0px 0px 65px;'>\s*(.*?)\s*<iframe src=#igs);
