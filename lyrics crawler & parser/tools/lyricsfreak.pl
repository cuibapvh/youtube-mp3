...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /print\.php/ig ){	
...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				my ( $title, undef, $artist, undef, undef, undef,undef,undef) = split('(\||-|:|by|By)', $title_temp);
				...
				my @split = split("\n", $string);				
				my ($content) = ($string =~ m#</a></h2>\s*(.*?)\s*Lyricsfreak.com &copy;#igs);
