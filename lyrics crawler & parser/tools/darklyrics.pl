...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /([a-z0-9])$/ig && -e $SourceFilename ){	
...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				my @split = split("\n", $string);
				my ($content) = ($string =~ m#<div id=lrc>\s*(.*?)\s*<div class=cmt>#igs);
		...
		#		$content = encode($enc, $songtext);
				...
				$hs->eof;
				my $c = "";
				foreach my $entry (split("<br />",$content )){
					$entry = encode($enc,$entry);
					$entry = trim($entry);
					#print "ID: $entry\n";
					next if ($entry =~ /(style|Submit|Ringtone|Meanings|\bad\b|cell)/ig);
					$c .= "$entry<br />\n";
				}
				$content = $c;
				$content = encode($enc, $content);
