...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /([a-z0-9])/ig && $SourceFilename !~ m/(page|feed)/ig && $size > 1000){	
...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				my ($title_temp1) = ($string =~ m#<meta name="keywords" content="\s*(.*?)\s*/>#igs);
				...
				my ($title2,,, ) = split(/,/ig, $title_temp1);
				my ($title,$artist) = split(/-/ig, $title2);
...
#				my ($title) = ($string =~ m#SongName = "\s*(.*?)\s*";#igs);
			...
			#	return;
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#Text:<BR><BR>\s*(.*?)\s*www\.amazon\.com#igs);
