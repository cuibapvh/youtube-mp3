...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				my ($cover) = ($string =~ m#"og:image" content="\s*(.*?)\s*"/>#igs);
				my ($title_temp) = ($string =~ m#<TITLE>\s*(.*?)\s*</TITLE>#igs);
				my ( $artist, undef, undef, undef,undef,undef) = split('(\||-|:|&raquo;)', $title_temp);
				...
				my @split = split("\n", $string);				
				my ($content) = ($string =~ m#class="songLyricsV14">\s*(.*?)\s*</div>#igs);
...
#
		...
		#	};
			...
			if ( $lenght>$req_lenght&&$lenght<2500000){
				...
				$content = strip_tags( $content, "<li><ul><ol><strong><b><h1><h2><h3><h4><h5><br><br /><br/>" );
				...
				if ( $lenght > $req_lenght ){
					...
					open(W,"+>>$logfile");
					print W "(r: $count) -> $lenght Bytes -> $title \n";
					print "(r: $count) -> $lenght Bytes -> $title \n";
					close W;
