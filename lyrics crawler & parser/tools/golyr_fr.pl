...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /\.html/ig && $SourceFilename !~ m/(page|feed)/ig && $size > 1000){	
...
#	};
			...
			#	$title = ucfirst($title);
				...
				my ( $title, undef, undef, undef, undef,undef,undef) = split(' et ', $title_temp);
...
#				my ($title) = ($string =~ m#SongName = "\s*(.*?)\s*";#igs);
			...
			#	return ;
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#<div id="lyrics">\s*(.*?)\s*addthis_toolbox addthis_default_style#igs);
