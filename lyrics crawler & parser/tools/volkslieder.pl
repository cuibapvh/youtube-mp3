...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /text+\.htnl/ig && $SourceFilename !~ m/(page|feed)/ig && $size > 1000){	
...
#				my ($title) = ($string =~ m#SongName = "\s*(.*?)\s*";#igs);
			...
			#	return ;
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#STYLE="margin-top: 0pt; padding-top: 0pt; margin-bottom: 0pt; padding-bottom: 2pt"\s*(.*?)\s*google_ad_type = "text_image";#igs);
