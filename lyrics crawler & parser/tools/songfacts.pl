...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /([a-z0-9])/ig && $size > 2000){	
...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				my ($title, $artist) = split(/ by /ig, $title_temp);
...
#				my ($title) = ($string =~ m#SongName = "\s*(.*?)\s*";#igs);
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#Lyrics Display START\s*(.*?)\s*Lyrics Display END#igs);
		...
		#		$content = encode($enc, $songtext);
				...
				#$artist = "";
				#my $ret = 0;
				foreach my $entry (split("<br />",$tmp_content )){
					$ccc++;
				#	print "$ccc) $entry\n"; next;
				...
				#	}
					next if ($entry =~ /(\bad\b|Writer|Licensed|Publishing|class=|LyricFind)/ig);
					$c .= "$entry<br />\n";
