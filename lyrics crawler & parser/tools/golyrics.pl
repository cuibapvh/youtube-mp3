...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /songtext-(\w+)-(\d+)\.html/ig ){	
...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				my ( $title1, undef, undef, undef, undef,undef,undef) = split('(\||by|By|Songtext)', $title_temp);
				...
				my @split = split("\n", $string);				
				my ($content) = ($string =~ m#<div id="lyrics"\s*(.*?)\s*addthis_toolbox addthis_default_style#igs);
