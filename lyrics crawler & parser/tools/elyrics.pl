...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /([a-z0-9])\.html/ig && -e $SourceFilename && $size > 100){	
...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				my ($title,$artist) = split(/ - /ig, $title1);
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#<div id='loading'></div>\s*(.*?)\s*<p><p><em><u>Official#igs);
		...
		#		$content = encode($enc, $songtext);
				...
				foreach my $entry (split("\n",$content )){
					...
					#print "ID: $entry\n";
					next if ($entry =~ /(\bad\b|cell|songteksten|elyrics|www|http)/ig);
					$c .= "$entry<br />\n";
				}
