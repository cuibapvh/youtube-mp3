...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /\.html/ig && -e $SourceFilename && $size > 100){	
...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#<div class="f-right" style="overflow:hidden">\s*(.*?)\s*/ringtones\.php\?artist=#igs);
				...
				#	}
					next if ($entry =~ /(\bad\b|term|ltd|lyricsty\.com)/ig);
					$c .= "$entry<br />\n";
