...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /-songtexte\.html/ig && $SourceFilename !~ m/(page|feed)/ig && $size > 1000){	
...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				my ( undef, $title1, undef, undef, undef, undef,undef,undef) = split('Lyrics:', $title_temp);
				my ( $title2, undef, undef, undef, undef,undef,undef) = split('und weitere', $title1);
				my ($artist,$title ) = split(/ - /ig, $title2);
...
#				my ($title) = ($string =~ m#SongName = "\s*(.*?)\s*";#igs);
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#<div style="font-size:13px; font-family:verdana;">\s*(.*?)\s*Alle Rechte liegen#igs);
		...
		#		$content = encode($enc, $songtext);
				...
				#	}
					next if ($entry =~ /(\bad\b|Writer|Licensed|Publishing|showads|">)/ig);
					$c .= "$entry<br />\n";
		...
		#	if ( $lenght>$req_lenght&&$count<=648577){
		#		$count++;
