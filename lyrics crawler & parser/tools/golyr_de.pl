...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /songtext/ig && $SourceFilename !~ m/(page|feed)/ig && $size > 1000){	
...
#	};
			...
			#	$title = ucfirst($title);
				...
				$title_temp =~ s/(Lyrics|Songtexte|Songtext|songtekst|Texter|Übersetzung|Paroles)//ig;
				...
				my ( $artist, $title, undef, undef, undef, undef,undef,undef) = split('-', $title);
...
#				my ($title) = ($string =~ m#SongName = "\s*(.*?)\s*";#igs);
			...
			#	return ;
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#<div id="lyrics">\s*(.*?)\s*<div style="border-top:1px solid \#ccc;border-bottom:1px solid \#ccc;margin-bottom:15px;padding:15px 0;text-align:center;">#igs);
