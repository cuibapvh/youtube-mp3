...
my $SourceFolder 	= '/home/songtexts/2/www.scoutsongs.com/lyrics/';
...
sub SourceStructure(){
	...
	# if ( $SourceFilename =~ /([a-zA-Z0-9])/ig && $SourceFilename !~ m/index/ig && $size > 2000){	
...
#				my ($title) = ($string =~ m#SongName = "\s*(.*?)\s*";#igs);
			...
			#	return ;
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#\#BeginEditable \"Lyric\"\s*(.*?)\s*\#BeginEditable \"Amazon\"#igs);
