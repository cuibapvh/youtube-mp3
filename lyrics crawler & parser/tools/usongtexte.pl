...
print "still to do;: /home/songtexts/neuigkeiten/www.songtexte.com/uebersetzung\n";
#$dbh->disconnect();
exit;
...
sub SourceStructure(){
	...
	#	print "SourceFilename=$SourceFilename\n";
		...
		if ($SourceFilename =~ m/PrintLyrics/ig){
...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				$title_temp =~ s/Lyrics auf Deutsch//ig;
				...
				my @split = split("\n", $string);				
				my ($content) = ($string =~ m#<div id="lyricsContent" class="songTranslation">\s*(.*?)\s*</p></div>#igs);
	...
	#				$entry = trim($entry);
	#				next if ($entry =~ /(style|Submit|Ringtone|\:)/ig);
	#				$c .= "$entry<br />\n";
	#			}
	#			$content = $c;
	...
	#		if ( $lenght>$req_lenght&&$count<=151882){
	#			$count++;
