...
my $message = Email::MIME->create(
  header_str => [
    ...
    Subject => "$scriptname Ready with install at " . localtime(),
  ],
  attributes => {
    encoding => 'quoted-printable',
    charset  => 'ISO-8859-1',
  },
  body_str => "$scriptname Ready with install! Please setup next lyrics crawler.\n",
);
...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /([a-z0-9])/ig && -e $SourceFilename && $size > 100){	
...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				$title_temp =~ s/(Lyrics|Songtexte|Songtext|songtekst)//ig;
				$title_temp =~ s/&#(\d+);//gi;
				
				my ( $title1, undef, undef, undef, undef,undef,undef) = split('(\|)', $title_temp);
				my ($artist, $title) = split(/ - /ig, $title_temp);
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#<div style="float: left; min-width: 560px;">\s*(.*?)\s*</div>#igs);
		...
		#		$content = encode($enc, $songtext);
				...
				foreach my $entry (split("\n",$content )){
					...
					next if ($entry =~ /(\bad\b|Artist|Album|Song|Typed|\:)/ig);
					$c .= "$entry<br />\n";
					
				}
				
				$content = encode($enc, $c);
				$title = "$artist - $title"; # temp
