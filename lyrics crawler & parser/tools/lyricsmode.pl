...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				my ( $title, undef, undef,undef,undef) = split('(\||-|:|&raquo;)', $title_temp);
				...
				$title =~ s/_/ /g;
				$title =~ s/([\w']+)/\u\L$1/g;
				$title =~ s/\.txt//ig; $title =~ s/\.php//ig; $title =~ s/\.html//ig;
				$title =~ s/[^[:ascii:]]/ /g;
				$title =~ s/&+(\w+)+;/ /g;
				$title =~ s/\s+/ /g;
				$title =~ s/&+(\w+)+;/ /g;
				...
				my ($content) = ($string =~ m#<p id="lyrics_text">\s*(.*?)\s*<p id="lyrics_text_selected">#igs);
			...
			#		
			#		$content = strip_tags( $content, "<li><ul><ol><strong><b><h1><h2><h3><h4><h5><br>" );
			#		$content =~ s/[^[:ascii:]]/ /g;
			#		$content =~ s/\s{2,}+/ /ig;
		...
		#	};
			...
			if ( $lenght>$req_lenght&&$lenght<2500000){
				...
				$content =~ s/\r\n/ /g;
				$content =~ s/&(\w+);//g;
				$content =~ s/(Version|Kommentare|Zu den kostenlosen|Diese Fickgeschichte wurde|Anmelden|Klicken|Trackback|Author|http|\(c\)|\@|Copyright|copy|Eigengebrauch|Einwilligung|Zuwiderhandlungen|Diese Geschichte wurde von|geschrieben|hinterlasse|veröffentlicht|Partner)//ig;
				$content =~ s/&#(\w+);//g;
				$content =~ s/(ero-geschichte\.(com|net|org|eu|tv|biz|ch|de|us|at|to|me|mobi))/nookiestar\.com/ig;
				
				$lenght = length($content);
				check();
				...
				if ( $lenght > $req_lenght ){
					...
					open(W,"+>>/home/songtexts/log/logging.txt");
					print W "Writing $lenght Bytes to Database -> (r: $count) ->: $title \n";
					print "Writing $lenght Bytes to Database -> (r: $count) ->: $title \n";
					close W;
				
					$count++;
					
					my $query = sprintf("%s (%s, %s, %s, %s)",
						"INSERT INTO songtexts (id,title,artist,songtext) VALUES",
						$dbh->quote(""),
						$dbh->quote( $title ),
						$dbh->quote( $artist ),
						$dbh->quote( $content ),							
