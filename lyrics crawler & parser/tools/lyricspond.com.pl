...
my $SourceFolder 	= '/home/songtexts/www.lyricspond.com/';
...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /lyrics-([a-zA-Z0-9])/ig && $SourceFilename !~ m/(page|feed)/ig && $size > 1000){	
...
#				my ($title) = ($string =~ m#SongName = "\s*(.*?)\s*";#igs);
			...
			#	return ;
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#<img border="0" style="padding:0; margin-left: 5px; vertical-align: bottom;"\s*(.*?)\s*<div class="content_block comments">#igs);
