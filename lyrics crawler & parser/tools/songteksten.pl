...
my $message = Email::MIME->create(
  header_str => [
    ...
    Subject => "$scriptname Ready with install!",
  ],
  attributes => {
...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#<span itemprop="description">\s*(.*?)\s*</span></p>#igs);
		...
		#		$content = encode($enc, $songtext);
				...
				foreach my $entry (split("\n",$content )){
					...
					$entry = trim($entry);
					#print "ID: $entry\n";
					next if ($entry =~ /(style|Submit|Ringtone|Meanings|\bad\b|cell|songteksten)/ig);
					$c .= "$entry<br />\n";
				}
			...
			if ( $lenght>$req_lenght&&$count<=206329){
				$count++;
