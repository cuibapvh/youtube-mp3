...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /([a-z0-9])\.html/ig && $SourceFilename !~ m/index/ig && $size > 2000){	
	#	print "SourceFilename=$SourceFilename\n";
	#	return;
	
		if ($SourceFilename =~ m/index/ig){
			return;
		};
...
#				my ( $title1, undef, undef, undef, undef,undef,undef) = split(' Album: ', $title_temp);
				#my ($title, $artist) = split(/ - /ig, $title1);
								
				my ($artist) = ($string =~ m#ArtistName = "\s*(.*?)\s*";#igs);
				my ($title) = ($string =~ m#SongName = "\s*(.*?)\s*";#igs);
				
				return if ($title_temp =~ /(comment|by)/ig );
				
				#print "Title Eins: $title1\n";
				#print "Artist: $artist\n";
				#print "Title: $title\n";
				#return;
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#ringup_song\.js\s*(.*?)\s*ringdown_song\.js#igs);
		...
		#		$content = encode($enc, $songtext);
				...
				#	}
					next if ($entry =~ /(\bad\b|Thanks to|Other lyrics from)/ig);
					$c .= "$entry<br />\n";
