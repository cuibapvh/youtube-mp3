...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /index\.html/ig && $SourceFilename !~ m/(page|feed)/ig && $size > 2000){	
...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				my ( undef, $title1, undef, undef, undef, undef,undef,undef) = split('::', $title_temp);
				my ( $title2, undef, undef, undef, undef,undef,undef) = split('from', $title1);
				
				my ($artist,$title, undef ) = split(/ - /ig, $title2);
...
#				my ($title) = ($string =~ m#SongName = "\s*(.*?)\s*";#igs);
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#ads\.clicksor\.com\s*(.*?)\s*><P class=endvypis>#igs);
		...
		#		$content = encode($enc, $songtext);
				...
				#	}
					next if ($entry =~ /(\bad\b|Writer|Licensed|Publishing|showads|\.php)/ig);
					$c .= "$entry<br />\n";
