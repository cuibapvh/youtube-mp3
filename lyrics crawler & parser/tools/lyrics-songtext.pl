...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /index\.html\?p=/ig && $SourceFilename !~ m/(page|feed)/ig && $size > 1000){	
...
#	};
			...
			#	$title = ucfirst($title);
				...
				$title_temp =~ s/[^[:print:]]+/-/g;
				#my ($title_temp) = ($string =~ m#Artiest:\s*(.*?)\s*</p></li>#igs);
				
				my ( $title_tmp, undef, undef, undef, undef,undef,undef) = split('\|', $title_temp);
				my ( $artist, $title, undef, undef, undef, undef,undef,undef) = split('-', $title_tmp);
				my ( $artist2, $title2, undef, undef, undef, undef,undef,undef) = split('\&#8211;', $title_tmp);
				print "Aritist: $artist2\n";
				print "Title: $title2\n";
				return;
				
				
				if ( length($artist2) > length($artist) ){
					$artist = $artist2;
				}
				if ( length($title2) > length($title) ){
					$title = $title2;
				}
...
#				my ($title) = ($string =~ m#SongName = "\s*(.*?)\s*";#igs);
			...
			#	return ;
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m# <div class="art-postmetadataheader"><h2 class="art-postheader">\s*(.*?)\s*<div class="art-postmetadatafooter"><div class="art-postfootericons art-metadata-icons">#igs);
