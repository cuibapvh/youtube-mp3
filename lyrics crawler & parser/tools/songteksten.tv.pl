...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /\.php/ig && $SourceFilename !~ m/(page|feed)/ig && $size > 1000){	
...
#	};
			...
			#	$title = ucfirst($title);
				...
				my ( $artist, $title, undef, undef, undef, undef,undef,undef) = split('Song:', $title_temp);
...
#				my ($title) = ($string =~ m#SongName = "\s*(.*?)\s*";#igs);
			...
			#	return ;
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#<li style="width:400px;"><p>\s*(.*?)\s* <div id="rightCollumn">#igs);
