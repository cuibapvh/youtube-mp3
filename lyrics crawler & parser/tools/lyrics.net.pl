...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /([a-zA-Z0-9])/ig && $SourceFilename !~ m/(page|feed)/ig && $size > 1000){	
...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				my ( undef, $artist, undef, undef, undef, undef,undef,undef) = split('of', $title_temp);
...
#				my ($title) = ($string =~ m#SongName = "\s*(.*?)\s*";#igs);
			...
			#	return ;
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#<pre id="lyric-body-text"\s*(.*?)\s*<div class="lyric-credits clearfix">#igs);
		...
		#		$content = encode($enc, $songtext);
			...
			if ( $lenght>$req_lenght&&$count<=48827){
				$count++;
