...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /([a-z0-9])/ig && $SourceFilename !~ m/(page|feed)/ig && $size > 2000){	
...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				my ( $title1, undef, undef, undef, undef,undef,undef) = split('Songtext', $title_temp);
...
#				my ($title) = ($string =~ m#SongName = "\s*(.*?)\s*";#igs);
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#<div class="content">\s*(.*?)\s*<div class="field field-type-text field-field-lyrics-author">#igs);
