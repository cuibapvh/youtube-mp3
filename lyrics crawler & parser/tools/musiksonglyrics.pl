...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /songtext/ig && -e $SourceFilename && $size > 100){	
...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m# <div id="lyrics">\s*(.*?)\s*submit_rating\.php#igs);
		...
		#		$content = encode($enc, $songtext);
				...
				#	}
					next if ($entry =~ /(\bad\b|ringtone|received|based)/ig);
					$c .= "$entry<br />\n";
