...
my $SourceFolder 	= '/home/songtexts/3/www.lyricsbox.com/';
...
sub SourceStructure(){
	...
	if ( $SourceFilename =~ /([a-zA-Z0-9])-lyrics-([a-zA-Z0-9])/ig && $SourceFilename !~ m/(page|feed)/ig && $size > 1000){	
...
#	};
			...
			#	$title = ucfirst($title);
				...
				$title_temp =~ s/(Lyrics|Songtexte|Songtext|songtekst|Texter)//ig;
				#$title_temp =~ s/&#(\d+);//gi;
...
#				my ($title) = ($string =~ m#SongName = "\s*(.*?)\s*";#igs);
			...
			#	return ;
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#<TD><DIV style="font:12px arial">\s*(.*?)\s*<DIV id=adslot_69_1 class=advertisment></DIV>#igs);
