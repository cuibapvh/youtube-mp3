...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				my ( $title1, undef, undef, undef, undef,undef,undef) = split('(\||by|By|\:)', $title_temp);
				...
				my @split = split("\n", $string);				
				my ($content) = ($string =~ m#<div id="main" unselectable="on">\s*(.*?)\s*<div id="main_bot">#igs);
	...
	#		if ( $lenght>$req_lenght&&$count<=9961){
	#			$count++;
