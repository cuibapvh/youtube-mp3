...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				my ($cover) = ($string =~ m#<div><img src=\s*(.*?)\s*lyrics"/></div>#igs);
				...
				my @split = split("\n", $string);				
				my ($content) = ($string =~ m#<div id="lyrics-body-text">\s*(.*?)\s*<div id="selected-song-meaning-open" unselectable="on" style="display:none;">#igs);
	...
	#		if ( $lenght>$req_lenght&&$count<=4579){
	#			$count++;
