...
#	return ;
			...
			#	$title = ucfirst($title);
				...
				my ( $title, undef, undef, undef, undef,undef,undef) = split('(\||by|By|Songtexte)', $title_temp);
				my ($artist,$title) = split('-', $title_temp);
				...
				$title =~ s/(Lyrics|Songtexte)//ig;
				$title = trim($title);
								
				my @split = split("\n", $string);				
				my ($content) = ($string =~ m#<div id="lyr">\s*(.*?)\s*<h3>Binde diesen Code#igs);
		...
		#		$content = $songtext;
			...
			#			$c .= "$entry<br />\n";
			#		}
			#		$content = strip_tags( $content, "<li><ul><ol><strong><b></strong></b><h1><h2><h3><h4><h5><br><br /><br/>" );
			#		$content =~ s/[^[:ascii:]]/ /g;
			#		$content =~ s/\s{2,}+/ /ig;
			...
			if ( $lenght>$req_lenght&&$count<=7626){
				$count++;
