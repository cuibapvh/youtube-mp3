...
my $SourceFolder 	= '/home/songtexts/new/songtext/';
...
#	};
			...
			#	$title = ucfirst($title);
				...
				$title_temp =~ s/(Lyrics|Songtexte|Songtext|songtekst|Texter|Übersetzung|von)//ig;
				#$title_temp =~ s/&#(\d+);//gi;
...
#				my ($title) = ($string =~ m#SongName = "\s*(.*?)\s*";#igs);
			...
			#	return ;
				...
				my @split = split("\n", $string);
				my ($content1) = ($string =~ m#<div id="lyrics">\s*(.*?)\s*<div class="translationsList">#igs);
